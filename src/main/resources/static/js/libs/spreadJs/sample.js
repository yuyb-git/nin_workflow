...
function syncSpreadPropertyValues() {
    var options = spread.options;
    // General
    options.allowUserDragMerge=true;
...
function deleteInsertComment(){
    $.each(spread.contextMenu.menuData, function (p, v) {
        if(v){
            if (v.name === "gc.spread.insertComment") {
...
function initSpread() {
    ...
    var bgBillId = temp.bgBillId;
    var _url = '';
    var _data = '';
    if(excelId){
        _url = '../../lsptJcExcel/selectExcel';
        _data = {"excelId":excelId};
    }else {
        _url = '../../getExcelContent';
        _data = {"bgBillId":bgBillId};
    }
    $.ajax({url:_url,type:"get",async:false,data:_data,success:function(data){
        // var spread2 = $("#ss").data("workbook");
        spread.options.allowUndo = true;
        spread.options.allowCopyPasteExcelStyle = true;

        // spread2.bind(spreadNS.Events.ActiveSheetChanged, function (e, args) {
        //     console.log("oldSheet.name:"+args.oldSheet.name());
        //     console.log("newSheet.name:"+args.newSheet.name());
        // });


        deleteInsertComment();
        if(data.data.excelId){
            spread.fromJSON(JSON.parse(pako.inflate(atob(data.data.init),{to: 'string'})));

            var totalWeight = 0;
            // console.log(typeof data.data.answerContent);
            var json=data.data.answerContent;
            if(json&&typeof json !="object"){
                json = JSON.parse(json);
            }
            for(var sheetName in json) {
                var gcSheet = spread.getSheetFromName(sheetName);
                if (gcSheet) {
                    gcSheet.suspendPaint();
                    var array = json[sheetName];
                    var len = array.length;
                    for (var row = 0; row < len; row++) {
                        var x = array[row].x;
                        var y = array[row].y;
                        var answer = array[row].setting.answer;
                        var weight = array[row].setting.weight;
                        var key = array[row].setting.key;
                        var dim_answer = array[row].setting.dim_answer || "";
                        if(dim_answer == undefined || dim_answer =="undefined"){
                            dim_answer = "";
                        }
                        var dim_answer_str = "&" + dim_answer;
                        if(dim_answer==""){
                            dim_answer_str = "";
                        }
                        gcSheet.setValue(x, y, "answer@" + answer + "#" + weight + "*" + key + dim_answer_str);
                    }
                    gcSheet.resumePaint();
                }
            }
        }else {
            var str = JSON.parse(data.data.fieldA);
            spread.fromJSON(JSON.parse(pako.inflate(atob(str.a5),{to: 'string'})));
        }
        spread.getActiveSheet().setSelection(0,0,1,1);
        var count = spread.getSheetCount();
        for ( var row=0 ; row < count ; row++ ){
            var floatingObjects = spread.getSheet(row).floatingObjects.all();
            spread.getSheet(row).options.isProtected = false;
            for(var floatIndex = 0; floatingObjects[floatIndex];floatIndex++){
                floatingObjects[floatIndex].isVisible(false);
            }
        }
...
function addPicture(pictureUrl) {
    ...
    if (pictureUrl !== "" && sel) {
        ...
        var name = "Picture" +(new Date()).valueOf()+ pictureIndex;
        pictureIndex++;
...
function attachToolbarItemEvents() {
    ...
    $("#addcomment").click(function () {
        ...
        var comment = sheet.comments.add(row,column,new Date().toLocaleString());
        ...
        if(confirm("确认清除答案？")) {
            ...
            for (var x = 0; x < rowCount; x++) {
                for (var y = 0; y < columnCount; y++) {
                    var answer = $.trim(activeSheet.getValue(x, y));
                    if (answer.indexOf("answer") >= 0) {
                        activeSheet.setValue(x, y, "");
                    }
                }
            }
        }else{
...
function setFloat(tempnum){
    var re = /^-?\d*\.?\d*$/;
    if(!re.test(tempnum)) {
        tempnum = tempnum.replace(/[\r\n]/g,"");
        return tempnum;
    }else{
        return parseFloat(tempnum).toFixed(2);
    }
    /*else{
     var red=/^(0|[1-9][0-9]*|-[1-9][0-9]*)$/;
     if(!red.test(tempnum)){
...
function jsonList(){
    ...
    var jsonObj = {};
    for(var x=0;x<sheetCount;x++) {
        var sheet = spread.getSheet(x);
        sheet.suspendPaint();
        var sheetName = sheet.name();
        var array = [];
        for (var i = 0; i < sheet.getRowCount(); i++) {
            for (var j = 0; j < sheet.getColumnCount(); j++) {
                if (sheet.getValue(i, j)&&typeof sheet.getValue(i, j) == "string") {
                    if(sheet.getValue(i, j).trim().indexOf("answer")==0){
                        var length = sheet.getValue(i, j).length;
                        var cellContent = sheet.getValue(i, j);
                        // console.log("cellContent:"+cellContent);
                        var indexAnswer = cellContent.indexOf("\@");
                        var indexWeight = cellContent.indexOf("\#");
                        var indexKey = cellContent.indexOf("\*");
                        var indexDimAnswer = cellContent.indexOf("\&");
                        var dim_answer = "";
                        if(indexDimAnswer >=0 ){
                            dim_answer = cellContent.substring( indexDimAnswer + 1,cellContent.length );
                            // dim_answer_str = ",\"dim_answer\":\"'"+dim_answer ;
                        }else{
                            indexDimAnswer = cellContent.length;
                        }

                        var answer = cellContent.substring( indexAnswer + 1,indexWeight );
                        var weight = cellContent.substring( indexWeight + 1,indexKey );
                        var key = cellContent.substring ( indexKey + 1,indexDimAnswer );
                        // answer = setFloat(answer);
                        // weight = setFloat(weight);
                        // key = setFloat(key);

                        var itemObj = {};
                        var setting = {};
                        setting.answer = answer;
                        setting.dim_answer = dim_answer;
                        setting.weight = weight;
                        setting.key = key ;
                        itemObj.x = i ;
                        itemObj.y = j;
                        itemObj.setting = setting;
                        array.push(itemObj);
        ...
        sheet.resumePaint();
        jsonObj[sheetName] = array;
    }
    console.log("josnObj:"+JSON.stringify(jsonObj));
...
function exportToJSON() {
    ...
    var json = spread.toJSON({includeBindingSource: true}),
        _text = JSON.stringify(json);
    _text = btoa(pako.deflate(_text, {to: "string"}));
    var bgBillId = temp.bgBillId;
    var _url = '';
    var _data = '';
    if(excelId){
...
//签章
function drawCachet(data){
    // var roleId = "";
    // var questionId = "";
    // var _data={};
    // _data.roleId = roleId ;
    // _data.questionId = roleId ;
    // var url = epcUrlSet.SX.GET_STUDENT_QUESTION_ROLE_CACHET;
    // $.ajax({url:url,type:"post",async:false,data:_data,succes:function (result) {
    //
    ...
    var w=50;
    var h = 25;
    var cachet = $('<div class="billCachet billCachetPageNo' + x + '" id="gzBillCachet1' + x + '"><span class="cacheClose"></span></div>');
    cachet.css({
        position: 'absolute',
        left: '50%',
        top: '50%',
    ...
    // var wnd = $("#bigframe")
    draw_gz("gzBillCachet10", window, 2, "孙建勇", "", "0px", "", "0px", 50, 25, "13px", "RED", "", 0);
    // draw_gz("gzBillCachet" + x, Bill[role].BillWindow, cachetShape, cachetContentMid, cachetContentArc, cachetFontArc, cachetContentBottom, cachetFontBottom, w, h, cachetFontMid, color, dynamicText, yujiao);



    var billObj = $('#ss');

    var startX = 0, // 第一次点击的位置
        startY = 0,
        width = billObj.outerWidth(),
        height = billObj.outerHeight(),
        isClick = false,
        x = 0, // 当前签章位置
        y = 0;

    var currentObj = $("#gzBillCachet10"),
        cacheW = parseInt(currentObj.css('width')),
        cacheH = parseInt(currentObj.css('height'));

    currentObj.mousedown(function (e) {
        // layer.msg("鼠标按下了");
        e.stopPropagation();
        e.preventDefault();
        isClick = true;
        ...
        x = parseInt(currentObj.css('left'));
        y = parseInt(currentObj.css('top'));

        $(document).mousemove(function (e) {
            e.stopPropagation();
            e.preventDefault();
            if (isClick == true) {
                var moveX = x + (e.pageX - startX);
                var moveY = y + (e.pageY - startY);
                if ((moveX > width - cacheW) || (moveY > height - cacheH) || (moveX < 0) || (moveY < 0)) {
                    layer.alert('签章不可超出范围');
                    $(document).off("mousemove");
                }
                moveX = (moveX >= width - cacheW) ? width - cacheW : moveX;
                moveY = (moveY >= height - cacheH) ? height - cacheH : moveY;
        ...
        currentObj.mouseup(function (e) {
            e.stopPropagation();
            e.preventDefault();
            // var moveX = x + (e.pageX - startX);
            // var moveY = y + (e.pageY - startY);
            // $(this).attr("x",parseInt($(this).css('left'))) ;
            // $(this).attr("y",parseInt($(this).css('top'))) ;
            // console.log("display:"+currentObj.css("display"));
            // console.log("x:"+currentObj.css("left"));
            // console.log("y:"+currentObj.css("top"));
            // console.log("z-index:"+currentObj.css("z-index"));
            // isClick =false;
            // layer.msg("鼠标放开了");
            isClick = false;
            $("body").off("mousemove");
    ...
    $(".billCachet").mouseover(function () {
        $(".cacheClose").show();
    });

    $(".billCachet").mouseleave(function () {
        $(".cacheClose").hide();
    });
